...
public class FutureMatchesCrawlService
{
        ...
        CancellationToken ct = default)
    {
        ...
        foreach (var game in gameConfigs) {
            ...
            try {
                ...
                var (logoRaw, bannerRaw) = await ExtractWikiMainPageVisualsAsync(mainDoc, mainUrl, ct)
    ...
    /// <summary>Any CSS <c>url(...)</c> (e.g. inside complex <c>background</c> / <c>background-image</c> values).</summary>
    ...
    /// <summary>
    /// Logo + banner from Main_Page: inline styles, embedded <c>&lt;style&gt;</c> and linked stylesheets
    /// (<c>.header-banner { background: url(...) }</c>), then fallbacks.
    /// </summary>
    private async Task<(string? LogoSrc, string? BannerSrc)> ExtractWikiMainPageVisualsAsync(
        IDocument doc,
        string mainPageUrl,
    ...
    /// <summary>Parses rules like <c>.header-banner { background: url(...) ... }</c> (shorthand or longhand).</summary>
